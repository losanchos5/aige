...
secondaryLayer: 4
order: 24
summary: "Firma cada artefacto de modelo en la compilación, adjunta la procedencia de compilación, rechaza formatos que ejecuten código, y verifica la firma y los resúmenes antes de que un runtime lo cargue."
---

# Patrón: Model Artefact Integrity

**Resumen:** Trata los pesos del modelo y sus ficheros acompañantes como artefactos de cadena de
suministro. Firma un manifiesto de cada fichero y su resumen cuando la compilación los produce,
adjunta la procedencia de compilación que dice qué los compiló a partir de qué entradas, prefiere
formatos de serialización que no puedan ejecutar código y escanea los que sí pueden, y haz que cada
runtime verifique la firma, los resúmenes y la procedencia contra la entrada del registro antes de
cargar un modelo. La verificación escribe un registro de evidencia, así que "¿qué bytes se
ejecutaron, y quién los produjo?" tiene una respuesta.

## Objetivos
Garantiza que el modelo que carga un servicio es el modelo que la pipeline compiló, evaluó y
registró, de un productor conocido, y que cargarlo no puede ejecutar código de un atacante.
...
Registra y limita cada actor antes de que actúe; construye el control en el punto más temprano en
que puede bloquear; instrumenta la compilación para que produzca su propia prueba.

## Contexto
Los modelos se mueven desde trabajos de entrenamiento a registros a clusters de servicio, se ajustan
desde bases extraídas de hubs públicos, y se copian entre entornos y regiones. Los ficheros son
binarios grandes que nadie lee. Los formatos comunes no son inertes: la documentación de Python
advierte que "El módulo pickle no es seguro. Solo desempaqueta datos en los que confías." [1], y
Hugging Face describe "ataques peligrosos de ejecución de código arbitrario" que pueden ejecutarse
cuando se carga un fichero pickle [2].

## Problema
La pipeline evalúa un modelo y la producción puede cargar otro.

- **Fuerzas.** Los equipos extraen pesos por nombre o etiqueta, y una etiqueta puede moverse. El
  escaneo ayuda pero, como dice Hugging Face de su propio escáner de importación pickle, no es "100%
  infalible" [2]. El Reglamento de IA de la UE espera que los sistemas de alto riesgo resistan
  ataques en "componentes preentrenados utilizados en el entrenamiento (envenenamiento de modelos)"
  (`Art. 15(5)`) [3]. MITRE ATLAS cataloga el compromiso de la cadena de suministro del modelo en sí
  (`AML.T0010.003`) y la ejecución por el usuario de artefactos de IA inseguros (`AML.T0011.000`)
  [4], y OWASP lista la cadena de suministro como `LLM04:2026` [5] y las vulnerabilidades de cadena
  de suministro de agentes como `ASI04` [6].
- **Modo de fallo.** Un fichero de modelo manipulado o malicioso se carga con los privilegios del
  proceso de servicio. O un modelo que saltó la puerta de eval llega a producción a través de una
  copia manual. Después de un incidente, nadie puede probar qué pesos produjeron los resultados en
  cuestión.

## Solución
Firma en compilación, prueba cómo se compiló, carga solo formatos seguros, y verifica antes de
cargar.

1. **Firma en compilación.** Produce un manifiesto que lista cada fichero de modelo y su resumen
   criptográfico y firma el manifiesto. La especificación OpenSSF Model Signing (OMS), introducida
   por OpenSSF en junio de 2025, hace exactamente esto: una firma separada sobre un manifiesto de
   hashes de ficheros, en el formato de paquete Sigstore, y agnóstica de PKI (PKI empresarial,
   certificados autofirmados, claves desnudas o Sigstore sin claves) [7]; su implementación de
   referencia es la biblioteca `model-signing`} y CLI en el repositorio model-transparency de
   sigstore, cuya verificación recomputa los hashes [8].
2. **Adjunta la procedencia de la compilación.** Registra qué compiló el artefacto, mediante qué
   proceso y desde qué entradas de nivel superior, como procedencia SLSA
   ...
   vulnerabilidad «más allá de las capacidades de la mayoría de adversarios») [9]. Incluye el
   resumen del modelo base y los registros de admisión del conjunto de datos entre las entradas, y
   enumera los mismos artefactos en el [AIBOM](/patterns/aibom).
3. **Prefiere formatos seguros; escanea el resto.** Almacena pesos como safetensors, "un nuevo
   formato simple para almacenar tensores de forma segura (a diferencia de pickle)" [10]. Donde un
   framework aún carga pickle, mantén sus restricciones activadas: el {`torch.load`} de PyTorch por
   defecto es {`weights_only=True`} en su documentación actual y advierte "Nunca cargues datos de
   una fuente que no sea de confianza" [11]. Escanea cada fichero serializado para importaciones que
   ejecuten código antes de que llegue a un registro, y pone en cuarentena lo que falla.
4. **Fija modelos de terceros por resumen.** Extrae por resumen de contenido, no por etiqueta;
   verifica la firma del editor donde exista una; escanea y re-aloja el artefacto en el registro
   interno; y registra la fuente ascendente y el resumen en la entrada del registro.
5. **Verifica antes de cargar.** El control de admisión de la plataforma de servicio rechaza un
   modelo cuya firma, identidad del firmante, resúmenes de ficheros o procedencia no coincidan con
   la entrada del registro para la versión que se está desplegando, y escribe un registro de
   evidencia de cualquier forma. ATLAS lista la firma de código (`AML.M0013`), la verificación de
...
El AI RMF pide que los modelos preentrenados utilizados para el desarrollo sean "monitoreados como
parte del monitoreo y mantenimiento regular del sistema de IA" (MANAGE 3.2) y que la seguridad y la
resiliencia sean "evaluadas y documentadas" (MEASURE 2.7) [12]. Los proveedores de modelos de uso
general con riesgo sistémico deben garantizar "un nivel adecuado de protección de ciberseguridad"
para el modelo y su infraestructura física (`Art. 55(1)(d)`}) [3].
...
> **Ejemplo (ilustrativo)** Un equipo de plataforma descubrió que tres servicios cargaban el "mismo"
> modelo ajustado desde tres cubos diferentes, uno copiado a mano meses antes. Movieron la firma a
> la pipeline de entrenamiento, hicieron que el controlador de admisión de servicio verificara el
> manifiesto y la procedencia contra la entrada del registro, y convirtieron los puntos de control
> pickle restantes a safetensors. Los registros de evidencia de la primera semana mostraron un
> despliegue rechazado en una discrepancia de resumen: el modelo copiado a mano, que nunca había
> pasado la puerta de eval actual.

## Consecuencias
Lo que se ejecuta es demostrablemente lo que se compiló y evaluó, los ataques de cadena de
suministro en el modelo tienen que derrotar una firma y una verificación de procedencia en lugar de
una copia de fichero, y cada carga deja evidencia. Los costos: gestión de claves o una dependencia
de Sigstore; trabajo de plataforma de compilación para alcanzar niveles SLSA más altos; conversión
de puntos de control heredados; y latencia de verificación en carga, que es pequeña comparada con
los tiempos de carga del modelo pero debe presupuestarse para un escalado rápido.
...
**Correspondencias:** Reglamento de IA de la UE Art. 15(5), Art. 55(1)(d) · ISO/IEC 42001 A.6.2.5,

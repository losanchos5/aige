...
- **Two variants, one layout.** The **web** variant colours by class (light/dark from the tokens)
  ...
  In the portrait, the Patterns branch renders the name of every pattern in `patterns.ts` as
  layer-coloured chips (`portrait: 'chips-only'` on the layer nodes) instead of repeating the five
  layer headers that The Stack branch already shows; its chips stay ≥ 14px so they survive the
  frame's scale-to-fit.

...
- [x] 11.5 (2026-09-24: distill 80ae443 mergeada, props retirados, cadena completa verde: 548 default, 116 a11y, lhci ≥ 0,97/1/1/1, detector 62; PRs apilados abiertos, merge a main pendiente de Jordi) Integración en `feat/ui-audit-fixes` (worktree `.claude/worktrees/audit`): merge de las cuatro ramas, `npm run build` → `npm test` → `test:a11y` → `test:visual` (regenerar baselines una sola vez) → `lhci`; `code-reviewer`; PR contra `main`
